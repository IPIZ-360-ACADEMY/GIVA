...
function getSlaMeta(days) {
  if (days >= 6) {
    return { label: "SLA crÃ­tico", color: "#b91c1c", background: "#fee2e2" };
  }
  if (days >= 3) {
    return { label: "SLA atenÃ§Ã£o", color: "#92400e", background: "#fef3c7" };
  }
  return { label: "Dentro do SLA", color: "#166534", background: "#dcfce7" };
}

function getInternPhaseMeta(referenceDate) {
  const days = getPendingDays(referenceDate);
  if (days > 120) {
    return { label: "Potencial contratual", tone: "contract", days };
  }
  if (days > 30) {
    return { label: "EstÃ¡gio em desenvolvimento", tone: "active", days };
  }
  return { label: "Fase de integraÃ§Ã£o", tone: "onboarding", days };
}

function resolveInternAreaName(app) {
  return (
...
function resolveInternSectorName(app) {
  const vacancyTitle = String(app.vacancy?.title ?? "").trim();
  if (!vacancyTitle) return "Setor geral";

  const splitByDash = vacancyTitle.split(/[\-â€“â€”|/]/).map((segment) => segment.trim()).filter(Boolean);
